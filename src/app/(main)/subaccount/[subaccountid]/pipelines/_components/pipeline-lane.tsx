...
// @ts-ignore - Temporary ignore if IDE cache is stale, files exist on disk
import PipelineTicket from "./pipeline-ticket";
import LaneDetails from "@/components/forms/lane-details";
// @ts-ignore - Temporary ignore if IDE cache is stale, files exist on disk
import TicketDetails from "@/components/forms/ticket-details";
...
const PipelineLane: React.FC<PipelaneLaneProps> = ({
  ...
  const laneAmt = React.useMemo(() => {
    return tickets.reduce(
      (sum: number, ticket: TicketAndTags) => sum + (Number(ticket?.value) || 0),
      0
    );
  }, [tickets]);
                  ...
                  >
                    {(provided: DroppableProvided) => (
                      <div
                        ...
                        ref={provided.innerRef}
                      >
                        {tickets.map((ticket: TicketAndTags, index: number) => (
                          <PipelineTicket
